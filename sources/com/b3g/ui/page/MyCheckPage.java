...
public class MyCheckPage extends B3GPage {
    public MyCheckPage(Object obj, Object obj2, int i) {
    ...
    public Container GetPageContainer() {
        ...
        try {
            ...
            ArrayList checkProcess = getCheckProcess();
            ArrayList checks = getChecks((ServiceResponse) checkProcess.get(0));
            if (checks.size() > 0) {
                for (int i = 0; i < checks.size(); i++) {
                    ((MyCheckLcn) checks.get(i)).isSent = "0";
                    ((MyCheckLcn) checks.get(i)).isLcn = "1";
                }
            }
            ArrayList checks2 = getChecks((ServiceResponse) checkProcess.get(1));
            if (checks2.size() > 0) {
                for (int i2 = 0; i2 < checks2.size(); i2++) {
                    ((MyCheckLcn) checks2.get(i2)).isSent = "1";
                    ((MyCheckLcn) checks2.get(i2)).isLcn = "1";
                }
            }
            arrayList.add(new B3gContainer(getCheckForm(checks), "ENCAISSEMENT"));
            arrayList.add(new B3gContainer(getCheckForm(checks2), "PAIEMENT"));
    ...
    Container getCheckForm(ArrayList arrayList) {
        ...
        if (CihMBanking.sesPMTR.getIsDemo() == 0) {
            ...
            if (arrayList2.size() > 0) {
                ...
                container.addComponent(this.uiManager.GetCntMessage("Vous n'avez aucun chèque"));
            }
            return container;
        }
        container.addComponent(this.uiManager.GetCntMessage("Vous n'avez aucun chèque"));
    ...
    ArrayList getCheckProcess() {
        ...
        serviceRequest.setServiceId(900024);
