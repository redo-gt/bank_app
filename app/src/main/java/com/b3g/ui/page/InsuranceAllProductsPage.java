...
import com.b3g.services.Insurance;
...
public class InsuranceAllProductsPage extends B3GPage {
    DateFormat df;
    ...
    static /* synthetic */ int access$000(InsuranceAllProductsPage insuranceAllProductsPage, int i, int i2) {
        return insuranceAllProductsPage.getSeuilMontant(i, i2);
    }

    static /* synthetic */ boolean access$100(InsuranceAllProductsPage insuranceAllProductsPage, String str) {
        return insuranceAllProductsPage.getPasMontant(str);
    }

    static /* synthetic */ boolean access$200(InsuranceAllProductsPage insuranceAllProductsPage, Date date, Date date2, String str) {
        return insuranceAllProductsPage.CalculAge(date, date2, str);
    }

    static /* synthetic */ String access$300(InsuranceAllProductsPage insuranceAllProductsPage, String str) {
        return insuranceAllProductsPage.FillProductCode(str);
    }

    static /* synthetic */ String access$400(InsuranceAllProductsPage insuranceAllProductsPage, String str) {
        return insuranceAllProductsPage.FillPeriodicity(str);
    }

    public InsuranceAllProductsPage(Object obj, Object obj2, int i) {
    ...
    public Container GetPageContainer() {
        ...
        try {
            ...
            ServiceResponse allProductsResponse = insurance.getAllProductsResponse();
            ArrayList fetchProductsFromRespose = allProductsResponse.getStatusCode().equals("000") ? insurance.fetchProductsFromRespose(allProductsResponse) : null;
            Container container = new Container(new BoxLayout(2));
            container.setUIID("ad_CntAccountDetailsMain");
            ArrayList arrayList = new ArrayList();
            arrayList.add(new B3gContainer(getProducts(fetchProductsFromRespose), "Ma bancassurance"));
    ...
    class 2 implements ActionListener {
        ...
        public void actionPerformed(ActionEvent actionEvent) {
            if (this.val$monatntVersTxtF.getText().length() == 0 || this.val$codeProduitCombo.getSelectedIndex() == 0) {
                CihMBanking.sesPMTR.getCurrentUiManager().ShowMessageTransaction(InsuranceAllProductsPage.this.uiManager.stateMachine, "Veuillez saisir l'ensemble des champs obligatoires", null);
                return;
            }
            if (Integer.parseInt(this.val$monatntVersTxtF.getText()) < InsuranceAllProductsPage.access$000(InsuranceAllProductsPage.this, this.val$PreiodCombo.getSelectedIndex(), this.val$codeProduitCombo.getSelectedIndex())) {
                CihMBanking.sesPMTR.getCurrentUiManager().ShowMessageTransaction(InsuranceAllProductsPage.this.uiManager.stateMachine, "Le montant est inférieur au montant minimum autorisé. Veuillez vérifier le montant à transférer.", null);
                return;
            }
            if (!InsuranceAllProductsPage.access$100(InsuranceAllProductsPage.this, this.val$monatntVersTxtF.getText())) {
                CihMBanking.sesPMTR.getCurrentUiManager().ShowMessageTransaction(InsuranceAllProductsPage.this.uiManager.stateMachine, "Les valeurs des versements augmentent par palier de 100 dhs.", null);
                return;
            }
            if (InsuranceAllProductsPage.access$200(InsuranceAllProductsPage.this, this.val$dateEchePicker.getDate(), this.val$dateNaissPicker.getDate(), (String) this.val$codeProduitCombo.getSelectedItem())) {
                return;
            }
            InsuranceAllProductsPage.this.ShowcalcCapitalDetailPopUp((String) this.val$codeProduitCombo.getSelectedItem(), (String) this.val$PreiodCombo.getSelectedItem(), this.val$monatntVersTxtF.getText(), InsuranceAllProductsPage.this.df.format(this.val$datePremVersPicker.getDate()), InsuranceAllProductsPage.this.df.format(this.val$dateNaissPicker.getDate()), InsuranceAllProductsPage.this.df.format(this.val$dateEchePicker.getDate()), ((InssuranceSimulation) new InssuranceSimulation().InssuranceCotisationProcess(InsuranceAllProductsPage.access$300(InsuranceAllProductsPage.this, (String) this.val$codeProduitCombo.getSelectedItem()), InsuranceAllProductsPage.access$400(InsuranceAllProductsPage.this, (String) this.val$PreiodCombo.getSelectedItem()), this.val$monatntVersTxtF.getText(), InsuranceAllProductsPage.this.df.format(this.val$datePremVersPicker.getDate()), InsuranceAllProductsPage.this.df.format(this.val$dateNaissPicker.getDate()), InsuranceAllProductsPage.this.df.format(this.val$dateEchePicker.getDate())).get(0)).message + "");
            Date date = new Date();
            this.val$monatntVersTxtF.clear();
            this.val$codeProduitCombo.setSelectedIndex(0);
            this.val$PreiodCombo.setSelectedIndex(0);
            try {
                this.val$dateEchePicker.setDate(InsuranceAllProductsPage.this.df.parse(InsuranceAllProductsPage.this.df.format(date)));
                this.val$dateNaissPicker.setDate(InsuranceAllProductsPage.this.df.parse(InsuranceAllProductsPage.this.df.format(date)));
                this.val$datePremVersPicker.setDate(InsuranceAllProductsPage.this.df.parse(InsuranceAllProductsPage.this.df.format(date)));
    ...
    private Container getProducts(ArrayList arrayList) {
        Container container = new Container(new BoxLayout(2));
        container.setScrollVisible(false);
        container.setScrollableY(true);
        container.setFocusable(false);
        if (arrayList == null || arrayList.isEmpty()) {
            SpanLabel spanLabel = new SpanLabel("Vous n'avez aucun produit de Bancassurance.");
        ...
        for (int i = 0; i < arrayList.size(); i++) {
            container.addComponent(getProduct(((Insurance) arrayList.get(i)).codeProduitLabel, ((Insurance) arrayList.get(i)).referenceContrat, ((Insurance) arrayList.get(i)).codeProduit));
        }
        return container;
    }

    private Container getProduct(String str, String str2, String str3) {
        UIBuilder uIBuilder = new UIBuilder();
        Container createContainer = uIBuilder.createContainer("/cihv3", "InsuranceProductV1");
        createContainer.setScrollableY(false);
        createContainer.setScrollVisible(false);
        createContainer.setFocusable(false);
        ((SpanLabel) uIBuilder.findByName("ProductLbl", createContainer)).setText(str);
        ((SpanLabel) uIBuilder.findByName("ContractLbl", createContainer)).setText(str2);
        ((Button) uIBuilder.findByName("DetailsBtn", createContainer)).addActionListener(new 4(str2, str3));
        return createContainer;
    }

    class 4 implements ActionListener {
        final /* synthetic */ String val$codeProduct;
        final /* synthetic */ String val$contractNumber;

        4(String str, String str2) {
            this.val$contractNumber = str;
            this.val$codeProduct = str2;
        }

        public void actionPerformed(ActionEvent actionEvent) {
            InsuranceDetailPageV1 insuranceDetailPageV1 = new InsuranceDetailPageV1(InsuranceAllProductsPage.this.uiManager, InsuranceAllProductsPage.this.service, 116);
            insuranceDetailPageV1.setReference(this.val$contractNumber);
            insuranceDetailPageV1.setCodeProduct(this.val$codeProduct);
            InsuranceAllProductsPage.this.uiManager.NavigateToPage(insuranceDetailPageV1);
        }
    }
}
